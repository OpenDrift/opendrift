...
# MODEL SELECTION
###############################
o = OpenOil3D(loglevel=0, weathering_model = 'noaa') #weathering_model = 'noaa' # Using Oil information from NOAA's Oil Library
...
reader_roms_cnz_sea_temp = reader_netCDF_MetOcean.Reader(data_pth + 'current_roms_cnz_3D_20050101_20050107_temp_salt.nc',variables_to_use = ['dep','temp','salt'] ) #

reader_roms_cnz_3d_cur = reader_netCDF_MetOcean.Reader(data_pth + 'current_roms_cnz_3D_20050101_20050107_new.nc',variables_to_use = ['u','v'] ) #
# WAVES
...
o.add_reader([reader_basemap,reader_roms_cnz_3d_cur,reader_roms_cnz_sea_temp,reader_swan_nzra_surface,reader_wrf_nzra1_surface]) # 
# reader_roms_cnz_surface
...
o.seed_elements(lon, lat, 
                radius=10,
                number=100,
                time=reader_wrf_nzra1_surface.start_time,
                z=0.0,
                oiltype='DIESEL FUEL OIL (ALASKA)', # must be picked from available o.oil_types
                # m3_per_hour =***) # 
...
o.set_config('drift:current_uncertainty', 0.00)
...
o.set_config('processes:turbulentmixing',  False) # to run faster
...
###############################
# Running model
o.run(time_step = 1800, end_time = reader_wrf_nzra1_surface.start_time + timedelta(days = 7.0), outfile='opendrift_oil3d_all_inputs.nc',time_step_output = 1800)
