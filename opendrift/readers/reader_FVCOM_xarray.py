...
class Reader(BaseReader, UnstructuredReader):
    ...
    def __init__(self, filename, proj4, filetype="zarr", cache=None, name=None):
        ...
        if filetype == "zarr":
            ...
            if cache is not None:
                store = LRUStoreCache(
                    store_nocache, max_size=cache
                )  # e.g. 2**30 = 1 GiB cache
            else:
                store = store_nocache
            self.dataset = xr.open_dataset(
                store, engine="zarr", drop_variables=["time"]
            )
    ...
    def plot_mesh(self):
        ...
        plt.scatter(self.xc, self.yc, marker="o", color="red", label="centroids")
    ...
    def get_variables(self, requested_variables, time=None, x=None, y=None, z=None):
        ...
        * salinity
        ...
        logger.debug(
            "Requested variabels: %s, lengths: %d, %d, %d"
        ...
        all_avail = (len(node_variables) + len(face_variables)) == len(
            requested_variables
        )
        if not all_avail:
            logger.debug(
                f"Node variables {node_variables} and face variables {face_variables}"
            )
        ...
        if node_variables:
            ...
            for var in node_variables:
                ...
                if any([d.startswith("sig") for d in dvar.dims]):
                    # super hacky!
                    sigma_ind = xr.DataArray(
                        self.__nearest_node_sigma__(dvar, nodes.values, z.values),
                        dims=["particles"],
                    )
        ...
        if face_variables:
            ...
            for var in face_variables:
                ...
                if any([d.startswith("sig") for d in dvar.dims]):
                    # super hacky!
                    sigma_ind = xr.DataArray(
                        self.__nearest_face_sigma__(dvar, neles.values, z.values),
                        dims=["particles"],
                    )
                    assert len(sigma_ind) == len(z)
                else:
