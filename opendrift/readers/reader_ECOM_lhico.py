####
# author: @nilodna and @vic1309
# note: adapted from reader_netCDF_CF_generic.py
####
...
class Reader(BaseReader):
    ...
    def __init__(self, filename=None, name=None, proj4=None):
        ...
        if 'x' in locals() and 'y' in locals():
            # edition start for ECOM: made by @nilodna
            lon = self.Dataset['lon'][:]
            lat = self.Dataset['lat'][:]
            lon[lon == 0.] = np.nan
            lat[lat == 0.] = np.nan
            self.xmin, self.xmax = np.nanmin(lon),np.nanmax(lon)
            self.ymin, self.ymax = np.nanmin(lat),np.nanmax(lat)
            # edition final for ECOM: made by @nilodna
            ...
            self.x = lon  # Store coordinate vectors
            self.y = lat
        else:
