...
#
# Copyright 2021, Dr Julien Moreau, Plastic@Bay
...
class Reader(BaseReader, UnstructuredReader):
    ...
    <https://cfconventions.org/>`_ netCDF files using xarray. For multiple files,
    it is recommended to install Dask.
    ...
    Args:
        ...
        :param engine: engine used by xarray ({"netcdf4", "scipy", "pydap",
                       "h5netcdf", "pynio", "cfgrib", "pseudonetcdf", "zarr"} )
...
### if variable and dimension similar name, xarray crashes
        ...
        logger.info('Opening dataset: ' + str(filename))
        if type(filename) is xr.core.dataset.Dataset:
            logger.info('Opening existing xarray Dataset')
        ...
        self.x = self.dataset['node_x'].values
        self.y = self.dataset['node_y'].values
        # deactivated cell centered values
        ...
        self.times = self.dataset['time'].values.astype('M8[s]').astype('O')
        ...
        for var_name in self.dataset.variables:
            ...
            if var_name in [
                    'node_x', 'node_y', 'time', 'lon', 'lat', 'lonc', 'latc', 'siglay',
        ...
        #removed the compressed option as xarray use NaN. The array should then made into a masked array. Cannot see the interest for node coordinates they will always be there.
    ...
    def get_variables(self,
        ...
        * salinity
        ...
        # bugging with variables you can derive
        ...
        subds=self.dataset.sel(time=nearest_time)
        layer_flag=False
        ...
        if node_variables:
            ...
            if 'layer' in self.dataset.dims:
                pm = subds['altitude'].isel(node=nodes).values
                layer_ind= np.abs(pm - z).argmin(axis=0)
                layer_flag=True
            ...
            for var in node_variables:
                ...
                if layer_flag:
                # hack for Selafin
                    if dvar == 'sea_floor_depth_below_sea_level':
                         if 'sea_floor_depth_below_sea_level' in self.dataset.variables.keys():
                              variables[var]=subds[dvar].values[layer_ind,nodes]
                         else:
                              if 'altitude' in self.dataset.variables.keys():
                                  variables[var]=subds['altitude'].values[np.zeros(len(nodes)),nodes]
                              else:
                                  logger.debug("impossible to export bathymetry")
                    else:
                        logger.debug("exporting like there are layers")
                        variables[var]=subds[dvar].values[layer_ind,nodes]
                # sigma ind depends on whether variable is defined on sigma layer og sigma level
                elif 'siglev' in self.dataset[dvar].dims:
                    ...
                    logger.debug("exporting like there is no layers")
                    variables[var] = subds[dvar].sel(node=nodes).values
