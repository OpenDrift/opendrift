...
class Reader(BaseReader, StructuredReader):
    ...
    def __init__(self, dset=None, name=None, gridfile=None, standard_name_mapping={}):
        # dset: Intake catalog dataset (e.g. cat=intake.open_catalog(); dset=cat['my_dataset'])
        #          In other words, can be opened by  ds = dset.to_dask()
        if dset is None:
            raise ValueError('Need intake dataset as argument to constructor')
        ...
        try:
            ...
            logger.info('Opening dataset: ' + dset.name)
            dset.kwargs['decode_times'] = False
            self.Dataset = dset.to_dask()
            # self.Dataset = dataset.read_chunked()
