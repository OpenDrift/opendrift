...
try:
	...
	has_xarray = False
from opendrift.readers.basereader import BaseReader, vector_pairs_xy, pyproj
...
# Find horizontal indices corresponding to requested x and y
		...
		indx = np.floor((x-self.xmin)).astype(int) + clipped 
		indy = np.floor((y-self.ymin)).astype(int) + clipped  
		...
		if not hasattr(self, 'z') and (z is not None):  #if z is not in input, but is requested: 
			print ("z ==", z)
