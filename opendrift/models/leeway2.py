...
#
# Copyright 2015-2022, Knut-Frode Dagestad, MET Norway
...
class Leeway(OpenDriftSimulation):
        ...
        def seed_elements(self, lon, lat, object_type=None, **kwargs):
            ...
            if object_type is None:
                ...
                for object_type in range(1, len(self.leewayprop) + 1):
                    if self.leewayprop[object_type][
                            'OBJKEY'] == object_name or (
    ...
    class Simulation(Simulation):
        ...
        def update(self):
            ...
            winddir = np.arctan2(self.environment.x_wind,
                                 self.environment.y_wind)

            # Move particles with the leeway CCC TODO
            downwind_leeway = (
                (self.elements.downwind_slope + self.elements.downwind_eps /
                 20.0) * windspeed + self.elements.downwind_offset +
                self.elements.downwind_eps / 2.0) * .01  # In m/s
            crosswind_leeway = (
                (self.elements.crosswind_slope + self.elements.crosswind_eps /
                 20.0) * windspeed + self.elements.crosswind_offset +
            ...
            jib = jp_per_timestep > np.random.random(
